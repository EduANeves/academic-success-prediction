...
def loginaction(request):
    global em,pwd,email,nome,aviso
    aviso=''
    if request.method=="POST":
        ...
        c="select email from utilizador where username='{}' and password='{}'".format(em,pwd)
        cursor.execute(c)
        ...
        c = "update utilizador SET codigo='{}' where username='{}'".format(ran, em)
        cursor.execute(c)
        m.commit()

        if t==():
            aviso='Username ou password incorretas!'
            return render(request,'login_page.html',{'aviso': aviso})
        else:
            def decrypt(enc):
            ...
            #
            # f = "select nome from utilizador where username='{}' and password='{}'".format(em, pwd)
            # cursor.execute(f)
            #
            # g = cursor.fetchall()
            # for row in g:
            ...
            return HttpResponseRedirect("/verificar/?name={}".format(em))
